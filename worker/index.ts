	...
	try {
		...
		if (type === "mistake") {
			...
			if (!mistakes || mistakes.length === 0) {
				mistakesToUse = ["那", "哪", "拔", "拨", "拆", "折", "即", "既", "染", "梁"];
			}
			...
			prompt =
				type === "poem"
					? "生成一行适合儿童的唐诗或宋词，去除生僻字，长度10-20中文字。只输出文本，保留标点符号，不要包含其他解释。"
					: type === "tongue"
		...
		// Add random seed to prevent model caching identical responses
		prompt += ` [随机种子: ${Math.random().toString(36).slice(2, 8)}]`;
		...
		console.log("[Worker] Gemini Request Payload:", JSON.stringify(requestPayload));
